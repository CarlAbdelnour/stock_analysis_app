...
# Function to safely import packages with auto-install
def safe_import(package_name, import_name=None, pip_name=None):
    """Safely import a package, installing it if necessary"""
    ...
    try:
        ...
        st.warning(f"Installing {package_name} package...")
        if install_package(pip_name):
            st.success(f"{package_name} installed successfully!")
            return __import__(import_name)
        else:
            st.error(f"Failed to install {package_name}. Please try again later.")
            st.stop()

# Import required packages with auto-install
yf = safe_import("yfinance", pip_name="yfinance")
...
try:
    ...
    sklearn = safe_import("scikit-learn", "sklearn")
    from sklearn.model_selection import train_test_split
    from sklearn.linear_model import LinearRegression
    from sklearn.metrics import mean_squared_error, r2_score
