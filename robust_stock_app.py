...
def fetch_stock_data(symbol, start, end):
    ...
    try:
        ...
        st.write(f"🔍 Attempting to fetch data for {symbol} from {start} to {end}")
...
if st.sidebar.checkbox("🔧 Show Debug Info", value=False):
    ...
    st.sidebar.write(f"yfinance available: {YFINANCE_AVAILABLE}")
    st.sidebar.write(f"Current date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    if 'stock_data' in locals() and stock_data is not None:
        st.sidebar.write(f"Data shape: {stock_data.shape}")
        st.sidebar.write(f"Date range: {stock_data.index[0]} to {stock_data.index[-1]}")
        st.sidebar.write(f"Columns: {list(stock_data.columns)}")
    else:
