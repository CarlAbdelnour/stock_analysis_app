...
with col1:
    st.info(f"🐍 **Python**: {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}")
with col2:
...
with st.expander("🔧 Detailed System Information"):
    ...
    with col2:
        ...
        if YFINANCE_AVAILABLE:
            st.write(f"- **yfinance**: {yf_version} ✅")
        else:
            st.write(f"- **yfinance**: Not installed ❌")
        
        if REQUESTS_AVAILABLE:
            import requests
            st.write(f"- **requests**: {requests.__version__} ✅")
        else:
            st.write(f"- **requests**: Not installed ❌")
        
        if PLOTLY_AVAILABLE:
            import plotly
            st.write(f"- **plotly**: {plotly.__version__} ✅")
        else:
            st.write(f"- **plotly**: Not installed ❌")
        
        # Date and time info
        st.write(f"- **Current Time**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

# Python compatibility warnings
if sys.version_info < (3, 8):
    st.warning(f"⚠️ **Python {sys.version_info.major}.{sys.version_info.minor}** detected. yfinance works best with Python 3.8+")
elif sys.version_info >= (3, 12):
    st.info(f"ℹ️ **Python {sys.version_info.major}.{sys.version_info.minor}** - Very recent version. Some packages may have compatibility issues.")
else:
    st.success(f"✅ **Python {sys.version_info.major}.{sys.version_info.minor}** - Good compatibility with financial packages")
...
st.sidebar.write(f"🐍 **Python**: {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}")
...
if YFINANCE_AVAILABLE:
    if yf_version != "Unknown" and yf_version >= "0.2.60":
        ...
        st.sidebar.warning("⚠️ yfinance needs update")
    else:
        st.sidebar.info("ℹ️ yfinance version unknown")
else:
    st.sidebar.error("❌ yfinance not available")

# Python compatibility indicator
if sys.version_info < (3, 8):
    st.sidebar.warning("⚠️ Python version may cause issues")
elif sys.version_info >= (3, 12):
    st.sidebar.info("ℹ️ Very new Python version")
else:
    st.sidebar.success("✅ Good Python version")
...
if YFINANCE_AVAILABLE:
    python_status = "✅" if sys.version_info >= (3, 8) else "⚠️"
    yfinance_status = "✅" if yf_version >= "0.2.60" else "⚠️"
    ...
    if sys.version_info < (3, 8):
        st.warning("🔧 **Recommendation**: Upgrade to Python 3.8+ for better yfinance compatibility")
    
    if yf_version != "Unknown" and yf_version < "0.2.60":
        st.warning("🔧 **Recommendation**: Update yfinance with `pip install --upgrade yfinance`")
        
else:
    st.error(f"❌ **yfinance not available** - Install with: `pip install yfinance` | Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}")
